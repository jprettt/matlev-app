...
<div class="space-y-6">
    <header>
        ...
        <h1 class="mt-2 font-display text-3xl font-bold text-ink">Rekapitulasi Status Dokumen</h1>
    ...
    <section class="grid grid-cols-1 gap-4 sm:grid-cols-3">
        @foreach([['label' => 'Total Dokumen', 'value' => $uploads->count(), 'color' => 'text-ink', 'bg' => 'bg-white'], ['label' => 'Disetujui', 'value' => $uploads->where('status', 'approved')->count(), 'color' => 'text-emerald-700', 'bg' => 'bg-emerald-50'], ['label' => 'Menunggu / Revisi', 'value' => $uploads->whereIn('status', ['pending', 'rejected'])->count(), 'color' => 'text-amber-700', 'bg' => 'bg-amber-50']] as $stat)
            <div class="rounded-2xl border border-slate-200 {{ $stat['bg'] }} p-5 shadow-sm"><p class="text-xs font-bold uppercase tracking-wider text-slate-500">{{ $stat['label'] }}</p><p class="mt-2 text-3xl font-black {{ $stat['color'] }}">{{ $stat['value'] }}</p></div>
        @endforeach
    </section>

    <section class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        <div class="flex items-center justify-between border-b border-slate-100 px-5 py-4"><div><h2 class="font-display text-xl font-bold text-ink">Daftar Dokumen</h2><p class="mt-1 text-xs text-slate-500">Diurutkan dari upload terbaru.</p></div><span class="rounded-full bg-skywash px-3 py-1 text-xs font-bold text-ocean">{{ $uploads->count() }} dokumen</span></div>
