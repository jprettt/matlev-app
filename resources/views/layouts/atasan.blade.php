...
<head>
    ...
    <script>
        tailwind.config = {
            theme: { extend: { colors: { ink: '#172033', ocean: '#155e75', skywash: '#edf7fa' }, fontFamily: { display: ['Georgia', 'serif'] } } }
        }
    </script>
</head>
<body class="min-h-screen bg-[#f4f7f8] text-slate-900" x-data="{ sidebarOpen: false, collapsed: false }">
<div class="flex min-h-screen">
    <aside class="hidden lg:flex lg:flex-col bg-ink text-white transition-all duration-300" :class="collapsed ? 'w-20' : 'w-72'">
        <div class="flex items-center gap-3 border-b border-white/10 px-5 py-6" :class="collapsed ? 'justify-center px-3' : ''">
            <div class="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-cyan-400 text-lg font-black text-ink">M</div>
