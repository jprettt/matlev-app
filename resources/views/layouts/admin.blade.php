...
<div class="min-h-screen flex items-start">
    <aside class="bg-[#b89416] text-white flex flex-col sticky top-0 h-screen self-start shadow-2xl shadow-yellow-900/20 transition-all duration-300 hidden lg:flex" :class="sidebarCollapsed ? 'w-20' : 'w-64'">
        <div class="px-3 pt-3 pb-4 border-b border-yellow-200/20">
            ...
            <nav class="p-3 space-y-1.5">
                ...
                <a href="{{ route('admin.verifikasi') }}" title="Antrean Verifikasi" class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-base font-semibold transition {{ request()->routeIs('admin.verifikasi') ? 'bg-[#dfbd28] text-white shadow-lg shadow-yellow-950/20' : 'text-white hover:bg-[#caa51b]' }}" :class="sidebarCollapsed ? 'justify-center' : ''">
                    <svg class="w-5 h-5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg>
    ...
    <div x-show="sidebarOpen" x-transition class="fixed inset-0 z-50 lg:hidden" style="display: none;">
        ...
        <aside class="absolute left-0 top-0 h-full w-72 bg-[#b89416] text-white border-r border-yellow-200/20 p-4 space-y-2">
            ...
            <a href="{{ route('admin.verifikasi') }}" class="block px-3 py-2 rounded-lg text-base {{ request()->routeIs('admin.verifikasi') ? 'bg-[#dfbd28] text-white' : 'text-white hover:bg-[#caa51b]' }}">Antrean Verifikasi</a>
